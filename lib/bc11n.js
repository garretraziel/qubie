        ...
        function (document, callback) {
            ...
            socket.on('show_qr', function () {
                memdb.presenterEnabled(memstore, document.id, function (enabled, pres_id) {
                    if (enabled === true) {
                        var pres_url = url.resolve(config.link_url, '/document/p/' + pres_id);
                        QRCode.toDataURL(pres_url, function (err, qr) {
                            if (err) {
                                winston.error("during QR code generation: %s", String(err));
                            } else {
                                socket.emit('qrcode_url', qr);
                            }
                        });
                    }
                });
