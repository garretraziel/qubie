...
function onAuthorizeFail(data, message, err, accept) {
    if (err) {
        winston.error("during bc11n user authorization:", message);
    }
    accept(null, !err);
}
...
function rootComm(socket, data, db, memstore, drawstore, config) {
    async.waterfall([
        ...
        if (err) {
            winston.error("during bc11n root authorization:", err);
        }
    });
}
...
function listerComm(socket, data, db, memstore, s3bucket) {
    socket.request.user.getDocuments().success(function (documents) {
        ...
        winston.error('during getting documents from db for user %d:', socket.request.user.id , err);
    });
    socket.on('delete', function (document_id) {
        filemgr.deleteFile(s3bucket, document_id, socket.request.user, db, function (err) {
            if (err) {
                winston.error('during deleting document %d:', document_id , err);
...
function presenterComm(socket, data, db, memstore, drawstore, config) {
    async.waterfall([
        ...
        if (err) {
            winston.error("during bc11n presenter authorization:", err);
        }
    });
}
