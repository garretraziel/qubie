...
function presenterComm(socket, data, db, memstore) {
    socket.on('auth', function (name, passwd) {
        socket.join(data.document + ":auth"); // TODO: toto je nebezpecne. do prohlizece budou chodit hesla vsech dalsich
        socket.to(data.document + ":auth").emit({socket: socket, name: name, passwd: passwd});

        socket.on("auth_ok", function (auth_socket) {
            if (auth_socket === socket) {
