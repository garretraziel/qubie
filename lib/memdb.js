...
function getPage(memstore, id, default_page, done) {
    memstore.hget("document:" + id, "page", function (err, reply) {
        if (err) {
            winston.error("Err during reading page from memstore for document " + id + ":", err);
            done(default_page);
        } else {
            var page;
            if (reply === null) {
...
function incrOnline(memstore, id, done) {
    memstore.incr("document:" + id + ":online", function (err, count) {
        if (err) {
            winston.error("Err memstore during incr of " + id + ":", err);
            return done(0);
        }
        done(count);
    });
...
function decrOnline(memstore, id, done) {
    memstore.decr("document:" + id + ":online", function (err, count) {
        if (err) {
            winston.error("Err memstore during decr of " + id + ":", err);
            return done(0);
        }
        done(count);
    });
...
function getOnline(memstore, id, done) {
    memstore.get("document:" + id + ":online", function (err, count) {
        if (err) {
            winston.error("Err memstore during get of " + id + ":", err);
            return done(0);
        }
        if (count === null) {
            done(0);
...
function disablePresenter(memstore, id) {
    ...
    memstore.hget("document:" + id, "presenter_id", function (err, reply) {
        if (err) {
            winston.error("Err during reading presenter_id from memstore for document " + id + ":", err);
        } else if (reply !== null) {
    ...
    memstore.hget("document:" + id, "pdf_id", function (err, reply) {
        if (err) {
            winston.error("Err during reading pdf_id from memstore for document " + id + ":", err);
        } else if (reply !== null) {
...
function presenterEnabled(memstore, id, done) {
    ...
    memstore.hget("document:" + id, "presenter_id", function (err, reply) {
        if (err) {
            winston.error("Err during reading presenter from memstore for document " + id + ":", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
...
function getDocumentFromPresenter(memstore, presenter_id, done) {
    memstore.get("presenter:" + presenter_id, function (err, reply) {
        if (err) {
            winston.error("Err during reading presenter_id " + presenter_id + " from memstore:", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
...
function getDocumentFromPdfId(memstore, pdf_id, done) {
    memstore.get("pdf:" + pdf_id, function (err, reply) {
        if (err) {
            winston.error("Err during reading pdf_id " + pdf_id + " from memstore:", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
...
function getPdfId(memstore, document_id, done) {
    memstore.hget("document:" + document_id, "pdf_id", function (err, reply) {
        if (err) {
            winston.error("Err during reading pdf_id from memstore:", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
