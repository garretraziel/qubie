...
function getPage(memstore, id, default_page, done) {
    memstore.hget("document:" + id, "page", function (err, reply) {
        if (err) {
            console.error("Err during reading page from memstore for document " + id + ":", err);
            done(default_page);
        } else {
            var page;
            if (reply === null) {
...
function disablePresenter(memstore, id) {
    memstore.hget("document:" + id, "presenter_id", function (err, reply) {
        if (err) {
            console.error("Err during reading presenter_id from memstore for document " + id + ":", err);
        } else if (reply !== null) {
    ...
    memstore.hget("document:" + id, "pdf_id", function (err, reply) {
        if (err) {
            console.error("Err during reading pdf_id from memstore for document " + id + ":", err);
        } else if (reply !== null) {
...
function presenterEnabled(memstore, id, done) {
    ...
    memstore.hget("document:" + id, "presenter_id", function (err, reply) {
        if (err) {
            console.error("Err during reading presenter from memstore for document " + id + ":", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
...
function getDocumentFromPresenter(memstore, presenter_id, done) {
    memstore.get("presenter:" + presenter_id, function (err, reply) {
        if (err) {
            console.error("Err during reading presenter_id " + presenter_id + " from memstore:", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
...
function getDocumentFromPdfId(memstore, pdf_id, done) {
    memstore.get("pdf:" + pdf_id, function (err, reply) {
        if (err) {
            console.error("Err during reading pdf_id " + pdf_id + " from memstore:", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
...
function getPdfId(memstore, document_id, done) {
    memstore.hget("document:" + document_id, "pdf_id", function (err, reply) {
        if (err) {
            console.error("Err during reading pdf_id from memstore:", err);
            done(false);
        } else if (reply === null) {
            done(false);
        } else {
