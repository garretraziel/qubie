    ...
    return function (id, done) {
        db.User.find(id).success(function (user) {
            if (user !== null) {
                ...
                winston.error("Error during user retrieval: nonexistent id %d", id);
                done(new Error("Nonexistent user"));
            }
        }).error(function (err) {
            winston.error("Error with database during user retrieval: %s", String(err));
            done(err);
        });
    };
}
        ...
        if (user === null) {
            hashPassword(args.password, function (err, hash) {
                ...
                db.User.create({
                    ...
                    password: hash,
                    email: resolveBodyValue(args.email),
                    premium: resolveBodyValue(args.premium),
                    admin: resolveBodyValue(args.admin),
                    quota: resolveBodyValue(args.quota) || config.default_quota, // TODO: quota!
                    used_space: 0
                }).success(function () {
                    done(null);
                }).error(function (err) {
...
function changeUser(db, args, done) {
    db.User.find(args.database_id).success(function (user) {
        if (user === null) {
            done(new Error('Err: trying to edit non-existent user:' + args.database_id));
        } else {
            var updated = {};
            ...
            if (args.password !== "") {
                hashPassword(args.password, function (err, hash) {
                    ...
                    user.updateAttributes(updated, ["username", "email", "premium", "premium_to", "admin", "quota",
                        "password"]).success(function () {
                        done(null);
                    });
                });
            } else {
                user.updateAttributes(updated, ["username", "email", "premium", "premium_to", "admin",
                    "quota"]).success(function () {
                    done(null);
                });
            }
        }
